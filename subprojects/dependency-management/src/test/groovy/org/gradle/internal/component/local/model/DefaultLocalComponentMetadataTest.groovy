...
class DefaultLocalComponentMetadataTest extends Specification {
    ...
    def "can lookup configuration after it has been added"() {
        when:
        metadata.addConfiguration("super", "description", [] as Set, ["super"] as Set, false, false, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, taskDep)
        metadata.addConfiguration("conf", "description", ["super"] as Set, ["super", "conf"] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, taskDep)
    ...
    private addConfiguration(String name) {
        metadata.addConfiguration(name, "", [] as Set, [name] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, taskDep)
    }
    ...
    def "dependency is attached to module configuration and its children"() {
        ...
        when:
        metadata.addConfiguration("conf", null, [] as Set, ["conf"] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, Stub(TaskDependency))
        metadata.addConfiguration("child", null, ["conf"] as Set, ["conf", "child"] as Set, true, true, null,ConfigurationRole.CAN_BE_CONSUMED_ONLY,  Stub(TaskDependency))
        metadata.addConfiguration("other", null, [] as Set, ["other"] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, Stub(TaskDependency))
    ...
    def "builds and caches exclude rules for a configuration"() {
        given:
        metadata.addConfiguration("compile", null, [] as Set, ["compile"] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, Stub(TaskDependency))
        metadata.addConfiguration("runtime", null, ["compile"] as Set, ["compile", "runtime"] as Set, true, true, null, ConfigurationRole.CAN_BE_CONSUMED_ONLY, Stub(TaskDependency))
